...
@pytest.fixture()
def trees(tmp_path):
    """An access tree whose files are symlinks into a separate backup tree."""
...
def test_relative_real_file(trees):
    access, _ = trees
    assert to_relative_resource("saureus/vcf/S1_plain.vcf", access) == "saureus/vcf/S1_plain.vcf"


def test_relative_symlink_to_outside_target(trees):
    access, _ = trees
    assert (
        to_relative_resource("saureus/vcf/S1_freebayes.vcf", access)
...
def test_absolute_path_outside_base_rejected(trees):
    ...
    with pytest.raises(GenomeResourceError, match="Outside allowed directory"):
        to_relative_resource(str(backup / "saureus" / "vcf" / "S1_freebayes.vcf"), access)
...
def test_absolute_traversal_rejected(trees):
    ...
    with pytest.raises(GenomeResourceError):
        to_relative_resource(str(access / ".." / "backup" / "saureus"), access)
...
def test_served_path_stays_under_base(trees):
    """The file route gets the link path, not its target, and can read through it."""
    access, _ = trees
