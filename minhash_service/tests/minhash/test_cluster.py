...
from minhash_service.minhash.cluster import cluster_signatures, ClusterMethod

@pytest.mark.parametrize("sample_ids,cluster_method", [
...
def test_cluster_signatures(settings: Settings, sample_ids: list[str], cluster_method: ClusterMethod):
...
def test_cluster_single_signatures(settings: Settings):
    ...
    with pytest.raises(ValueError):
        cluster_signatures(sample_ids=["DRR237260"], method=ClusterMethod.SINGLE, cnf=settings)
