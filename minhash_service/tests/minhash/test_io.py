...
def test_add_signatures_to_index(settings_tmp_index: Settings):
    ...
    index_path = settings_tmp_index.signature_dir.joinpath(
        f"{settings_tmp_index.index_name}.sbt.zip"
    )
...
def test_remove_signatures_from_index(settings_tmp_index: Settings):
    ...
    # setup test
    resp = add_signatures_to_index(
        sample_ids=["DRR237260", "DRR237261"], cnf=settings_tmp_index
    )
    assert resp

    # perform test
    resp = remove_signatures_from_index(
        sample_ids=["DRR237261"], cnf=settings_tmp_index
    )
    assert resp
