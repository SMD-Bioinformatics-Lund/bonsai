import type { ApiJobStatus, ApiSampleQcStatus, ApiUserInfo, ApiGetSamplesDetailsInput, ApiSampleDetailsResponse, ApiJobSubmission, ApiClusterInput, ApiFindSimilarInput, GroupInfo, ApiToken } from "./types";
...
export class ApiService {
  ...
  login = async (userName: string, password: string) => {
    ...
    const url = `/token`;
    return this.http.request<ApiToken>(url, {
      method: "POST",
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded'
      },
      body: new URLSearchParams(authInfo).toString()
    });
  };
